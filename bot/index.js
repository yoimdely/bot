...
if (BOT_TOKEN) {
  ...
  bot.start(async (ctx) => {
    ...
    await ctx.reply('Добро пожаловать в Sex Health! Открой Mini App и начни вести статистику.\n/remind_on — включить напоминания\n/remind_off — выключить',
  ...
  cron.schedule('0 10 * * *', async ()=>{
    ...
    for (const chatId of subscribers) {
      try { await bot.telegram.sendMessage(chatId, 'Напоминание: обнови статистику 💚\nФакт дня: '+fact); } catch {}
    }
  });
  ...
  const port = process.env.PORT || 3000;
  app.listen(port, ()=> console.log('HTTP on', port, '(bot disabled — BOT_TOKEN not set)'));
}
